...
- 变现（2026-08-15 定稿）：**付款 + 邮箱激活码**，不走商店 IAP
  ...
  - **7 天免费试用**：安装即开始，试用期内全部 Pro 功能可用；到期后仅提醒一次升级
...
| WHO 生长曲线 | 锁定 | 可用 |
| 多设备同步 | 锁定 | 可用 |
| 医疗记录 / 学籍 / 好物 / 那年今日 | 锁定 | 可用 |

试用逻辑：首次启动记录试用开始时间（DataStore），7 天倒计时；到期后非 Pro 恢复锁定，并弹一次升级提醒（仅一次）。
- 合规包：docs/compliance/ 隐私政策、Google Play 家庭政策清单、国内发布清单

### 真机修复（2026-08-16）

- 提醒必达：精确闹钟权限不可用时自动兜底 setAlarmClock；真机排查确认 Flyme 需开启电池优化白名单（提醒设置页有引导）
- 成长时间轴：移除 ⭐/重要标签、节点卡片等高、支持编辑节点
- 相册：照片预览支持系统分享（FileProvider）
- 统计看板：月度概览、喂养 30 天趋势与类型占比、哭闹时段分布、待办完成率（聚合纯函数可单测）
- 宝宝自定义信息：新表 v5、宝宝详情页（模板添加/编辑/一键复制）、同步支持
- 待办：表单强区分（薄荷顶栏）、点击编辑、Checkbox 完成、滑动删除、支持选今天日期
- 共享同步：显示本机 IP 可复制、对方 IP 填写说明

### V2：金额记录

- 好物清单丰富化：分类（玩具/喂养/衣物/出行/医疗/其他）、价格、购买日期；列表显示分类与价格，顶部统计"共 N 件 · 总花费"
- 医疗记录：费用字段（就诊/买药花费），列表与详情显示
- 学籍阶段：学费 / 年字段，列表与详情显示
- 数据库 v4（保留迁移）；同步序列化同步新字段，旧数据兼容

### v0.1.0 新增（2026-08-27）

- 桌面小组件：Glance 实现，两种——照片 / 宝宝信息，长按桌面添加
- 疫苗提醒：VaccineReceiver 到点提醒，接入提醒链路
- 体验码机制：不绑定设备，任何手机输入即可激活永久 Pro（`我的 → 订阅升级 → 激活码`），README 首批 3 个永久码见文末
...
11. V2：好物加价格/分类 → 总花费 ¥899.00 统计正确；旧好物无价格兼容显示；v3→v4 迁移保留数据 ✅
